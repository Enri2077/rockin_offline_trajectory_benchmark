...
### TODO make it a real iteration, return with yield, be sure of the end condition (possible exceptions?)
...
#	def next_such_that(self, condition):
#		if condition(self.i1, self.i2):
#			yield self.i1, self.i2
...
class Error:
	...
	def update(self, robot_pose, mocap_pose):
		...
		#if self.avg_d:
		#	self.avg_d = self.avg_d*(self.n-1)/self.n + d/self.n
		#else:
		#	self.avg_d = d
		
		#if self.avg_l:
		#	self.avg_l = self.avg_l*(self.n-1)/self.n + l/self.n
		#else:
		#	self.avg_l = l
...
for f in os.listdir(mocap_bags_dir):
	...
	if mocap_bag_info and mocap_bag_info["start"] <= robot_bag_end and mocap_bag_info["end"] >= robot_bag_start: #TODO being sure of this 
...
for _, robot_pose, _ in robot_bag.read_messages("/rockin/"+teamname+"/marker_pose"):
	...
	try:
		...
		print "shortage of bags" #TODO explainalise
		sys.exit(5)
